...
export class ElasticsearchClient {
    ...
    private initMapping() {
        ...
        this.client.indices.putMapping({
            ...
            if (err) {
                Log.debug('Error while initing mapping for conversation mapping ', err);
            }
            else {
        ...
        this.client.indices.putMapping({
            ...
            if (err) {
                Log.debug('Error while initing mapping for message mapping ', err);
            }
            else {
        ...
        this.client.indices.putMapping({
            ...
            if (err) {
                Log.debug('Error while initing mapping for user mapping ', err);
            }
            else {
        ...
        this.client.indices.putMapping({
            ...
            if (err) {
                Log.debug('Error while initing mapping for context mapping ', err);
            }
            else {
        ...
        this.client.indices.putSettings({
            ...
            if (err) {
                Log.debug('Error while initing setting for index ', err);
            }
            else {
        ...
        return new Promise((resolve) => {
            ...
            this.client.search({
                ...
                if (err || !res || !res.aggregations || !res.aggregations.aggregatelist || !res.aggregations.aggregatelist.buckets) {
                    Log.debug('Error ', err, ' while aggregating with type ', type, ' body ', aggregate, ' with res ', res);
                    resolve();
                }
                else {
            ...
            this.client.search({
                ...
                if (err || !resp.hits || !resp.hits.hits) {
                    if (err.message == 'No Living connections' && option._retryCount < 6) {
                        option._retryCount++;
                        Log.debug('Error ' + err.message + ', retrying in 500ms for the ' + option._retryCount + ' times');
                        setTimeout(() => {
                            this.search(type, body, option).then(resolve).catch(reject);
                        }, 500);
                    }
                    else {
                        Log.debug("error ", err, " while querying type ", type, " with body ", body, " resp is ", resp);
                        resolve([]);
                    }
                }
                else {
    ...
    scroll(scrollId: string, array: ElasticsearchQueryResultDocument[], option: any): Promise<ElasticsearchQueryResultDocument[]> {
        ...
        return this.client.scroll({
            ...
            if (!resp || !resp.hits || !resp.hits.hits) {
                Log.debug("error while scrolling resp is ", resp);
    ...
    update(type: string, id: string, body: any, upsert: boolean, option?: any) {
        ...
        return new Promise((resolve) => {
            ...
            this.client.update({
                ...
                if (err)
                    Log.debug('Error while inserting ', id, ' of type ', type, ' to elasticsearch ', err);
                // else
    ...
    get(type: string, id: string, option?: any) : Promise<any>{
        if (!option) option = {};
        if (!option.isDocs) option.isDocs = false;
        return new Promise((resolve, reject) => {
            if (!this.client) {
                Log.debug('elasticsearch is not enabled, not getting ', type);
                throw new Error('elasticsearch is not enabled, not getting '+ type);
            }
            this.client.get({
                index: option.isDocs ? INDEX + '-docs' : INDEX,
                type: type,
                ...
                if (err && !response) {
                    Log.debug('Fatal error while getting id: ', id, ' of type ', type, ' from elasticsearch ', err, ' response ', response);
                    reject(err);
                }
    ...
    delete(type: string, id: string, option?: any) {
        ...
        return new Promise((resolve) => {
            ...
            this.client.delete({
                ...
                if (err)
                    Log.debug('Error while deleting ', id, ' of type ', type, ' from elasticsearch ', err, ' response ', response);
                else
